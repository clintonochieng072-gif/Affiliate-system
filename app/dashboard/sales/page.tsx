...
export default function SalesActivityPage() {
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-slate-400">Loading sales activity...</p>
        </div>
      </div>
    )
  }
  ...
  if (error || !data) {
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center">
        <p className="text-red-400">Failed to load sales activity</p>
      </div>
    )
  }
  ...
  const filteredSales = salesActivity.filter((sale: any) => {
    const matchesSearch = sale.userEmail.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         sale.productSlug.toLowerCase().includes(searchTerm.toLowerCase())
  ...
  return (
    <div className="min-h-screen bg-slate-950">
      ...
      <main className="lg:ml-64 p-4 lg:p-8">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-white mb-2">Sales Performance Analytics</h1>
          <p className="text-slate-400">Track all generated subscriptions and sales earnings</p>
        </div>
          ...
          {filteredSales.length === 0 ? (
            <div className="p-12 text-center">
              ...
              <h3 className="text-xl font-semibold text-slate-400 mb-2">
                {searchTerm || statusFilter !== 'all' ? 'No matching sales records' : 'No sales activity yet'}
              </h3>
              <p className="text-slate-500">
                {searchTerm || statusFilter !== 'all'
                  ? 'Try adjusting your filters'
                  : 'Start sharing your sales tracking links to generate activity'}
              </p>
            </div>
          ) : (
        ...
        {filteredSales.length > 0 && (
          <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
              ...
              <div className="text-2xl font-bold text-white">{filteredSales.length}</div>
            </div>
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
              <div className="text-sm text-slate-400 mb-1">Active Subscriptions</div>
              <div className="text-2xl font-bold text-green-400">
                {filteredSales.filter((s: any) => s.status === 'paid').length}
              </div>
            </div>
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
              <div className="text-sm text-slate-400 mb-1">Total Sales Earnings</div>
              <div className="text-2xl font-bold text-green-400">
                {formatCurrency(
                  filteredSales.reduce((sum: number, s: any) => sum + Number(s.salesEarnings), 0)
                )}
              </div>
            </div>
          </div>
